...
class UnsortedQueue(torch.nn.Module):
    ...
    def _load_from_state_dict(
        ...
        """Resize queue buffers to checkpoint shape before strict restoration.

        This keeps backward compatibility when older checkpoints stored a different
        placeholder shape for ``out`` (e.g., ``[N]`` vs ``[N, 1]`` before first append).
        """
        out_key = f"{prefix}out"
        if out_key in state_dict and self.out.shape != state_dict[out_key].shape:
