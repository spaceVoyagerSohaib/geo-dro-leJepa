...
Example:
    Using in a YAML config::
        ...
        module:
          ...
          forward:
            _target_: stable_pretraining.forward.simclr_forward
            _partial_: true
          backbone: ...
          projector: ...
...
def lejepa_forward(self, batch, stage):
    """Forward function for LeJEPA (Lean Joint-Embedding Predictive Architecture).

    LeJEPA learns representations through two complementary objectives:
    1. Prediction loss: Encourages invariance across different augmented views
       by minimizing the MSE between each view's projection and the mean projection.
    2. SIGReg loss: Prevents representation collapse by constraining embeddings
       to follow an isotropic Gaussian distribution.

    Unlike other methods, LeJEPA:
    - Does NOT require teacher-student networks or EMA
    - Does NOT require stop-gradients
    - Does NOT require negative samples or contrastive learning
    - Works with any number of views (typically 2 global + 6 local = 8 total)

    Args:
        self: Module instance (automatically bound) with required attributes:
            - backbone: Feature extraction network (any timm/torchvision model)
            - projector: Projection head mapping features to latent space
            - lejepa_loss: LeJEPALoss instance combining prediction + SIGReg
        batch: Either a dict of named views (from MultiViewTransform with named=True),
            a list of view dicts, or a single dict (for validation/single-view)
        stage: Training stage ('train', 'val', or 'test')

    Returns:
        Dictionary containing:
            - 'embedding': Feature representations from backbone [N*V, D] or [N, D]
            - 'loss': Combined LeJEPA loss (during training only)
            - 'label': Labels if present (for probes/callbacks)

    Note:
        Introduced in the LeJEPA paper :cite:`balestriero2025lejepa`.
        Reference: arXiv:2511.08544
    """
    out = {}
    ...
    if isinstance(batch, dict) and "image" not in batch:
        # Dict of named views: track global/local by key
        # Skip non-view entries (e.g., batch_idx)
        views = []
        global_mask = []
        for key, view in batch.items():
            ...
            if "global" in key:
                ...
                # Default to global if not explicitly labeled
                global_mask.append(True)
        if not views:
            raise ValueError("Multi-view batch did not contain any view dicts with 'image'")
    elif isinstance(batch, list):
        # List of views: assume first 2 are global
        views = batch
        n_global = min(2, len(views))
        global_mask = [i < n_global for i in range(len(views))]
    else:
        views = None

    if views is not None:
        # Multi-view training/evaluation
        # Process each view independently (supports mixed resolutions)
        embeddings = [
        ...
        if self.training:
            if not hasattr(self, "lejepa_loss"):
                raise ValueError(
                    "lejepa_forward requires 'lejepa_loss' to be provided "
                    "(e.g., stable_pretraining.losses.LeJEPALoss()). "
                    "Pass it when constructing the Module: "
                    "Module(..., lejepa_loss=LeJEPALoss(lambda_=0.02), ...)"
                )

            # Project embeddings per view
            projections = torch.stack(
                [self.projector(emb) for emb in embeddings], dim=0
            )

            global_mask_tensor = None
            if any(global_mask) and not all(global_mask):
                global_mask_tensor = torch.tensor(
                    global_mask, device=projections.device, dtype=torch.bool
                )

            total_loss, pred_loss, sigreg_loss = self.lejepa_loss(
                projections,
