...
class Module(pl.LightningModule):
    ...
    def _manual_optimization_handles(self):
        ...
        if len(optimizers) == 1 and len(schedulers) == 0:
            schedulers = [None]
        return optimizers, False

    def _step_manual_optimizers(self, optimizers, batch_idx: int, accum_steps: int):
        opt_step_idx = (batch_idx + 1) // accum_steps
        scheduler_configs = getattr(self.trainer, "lr_scheduler_configs", [])

        zero_grad_opts = []
        for idx, opt in enumerate(optimizers):
            name = self._optimizer_index_to_name[idx]
            if opt_step_idx % self._optimizer_frequencies[name] != 0:
                continue
            ...
            if idx < len(scheduler_configs):
                scheduler_cfg = scheduler_configs[idx]
                if getattr(scheduler_cfg, "interval", "step") == "step":
                    sched_freq = max(int(getattr(scheduler_cfg, "frequency", 1)), 1)
                    if opt_step_idx % sched_freq == 0:
                        self._step_scheduler(scheduler_cfg)
    ...
    def _step_scheduler(self, scheduler_cfg):
        ...
        if scheduler is None:
            return
        if getattr(scheduler_cfg, "reduce_on_plateau", False):
            monitor = getattr(scheduler_cfg, "monitor", None)
            if not monitor:
                logging.warning("ReduceLROnPlateau scheduler missing monitor key")
                return
            metric = self.trainer.callback_metrics.get(monitor)
            if metric is None:
                logging.warning(f"ReduceLROnPlateau monitor '{monitor}' not found")
                return
    ...
    def _step_epoch_schedulers(self, plateau_only: bool):
        scheduler_configs = getattr(self.trainer, "lr_scheduler_configs", [])
        if not scheduler_configs:
            return
        epoch_idx = self.current_epoch + 1
        for scheduler_cfg in scheduler_configs:
            if getattr(scheduler_cfg, "interval", "step") != "epoch":
                continue
            reduce_on_plateau = getattr(scheduler_cfg, "reduce_on_plateau", False)
            if plateau_only != reduce_on_plateau:
                continue
            sched_freq = max(int(getattr(scheduler_cfg, "frequency", 1)), 1)
            if epoch_idx % sched_freq != 0:
                continue
            self._step_scheduler(scheduler_cfg)

    def on_train_epoch_end(self):
        self._step_epoch_schedulers(plateau_only=False)

    def on_validation_epoch_end(self):
        self._step_epoch_schedulers(plateau_only=True)
    ...
    def on_train_start(self):
        ...
        for i in range(len(optimizers)):
            ...
            if name not in self._optimizer_frequencies:
                # Default to stepping every optimizer step unless explicitly configured.
                if hasattr(self, "optim"):
                    freq = self.optim.get("frequency", 1)
                else:
                    freq = 1
                self._optimizer_frequencies[name] = max(int(freq), 1)
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        if skipped_types:
            logging.warning(
                f"Skipped {len(skipped_types)} custom object(s) while offloading checkpoint. "
                "Set LOGURU_LEVEL=DEBUG to see details."
            )
            for obj_info in sorted(skipped_types):
                logging.debug(f"  - {obj_info}")
