...
class OnlineKNN(Callback):
    ...
    def setup(self, trainer: Trainer, pl_module: LightningModule, stage: str) -> None:
        ...
        if self._input_queue is None or self._target_queue is None:
            ...
            self._metric_num_classes = self._infer_metric_num_classes(
                pl_module.callbacks_metrics[self.name]["_val"]
            )

    def _infer_metric_num_classes(self, metrics: torch.nn.ModuleDict) -> Optional[int]:
        num_classes = None
        for metric in metrics.values():
            for attr in ("num_classes", "num_labels"):
                ...
                try:
                    ...
                    continue
                if num_classes is None or value > num_classes:
                    num_classes = value
        return num_classes
    ...
    def _compute_knn_predictions(
        ...
        if self._metric_num_classes is not None:
            num_classes = max(num_classes, self._metric_num_classes)
