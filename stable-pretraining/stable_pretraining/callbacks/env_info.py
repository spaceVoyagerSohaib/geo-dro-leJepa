...
class EnvironmentDumpCallback(Callback):
    ...
    def setup(self, trainer: pl.Trainer, pl_module: pl.LightningModule, stage) -> None:
        ...
        # 🔥 CRITICAL: Get root_dir in main thread BEFORE starting background thread
        ...
        if self.async_dump:
            ...
            self._dump_thread = threading.Thread(
                ...
                args=(root_dir,),  # Pass root_dir, not trainer!
                daemon=False,
            ...
            self._dump_environment(root_dir)
    ...
    def _get_snapshot_dirs(self, root_dir: Path) -> Dict[str, Path]:
        """Get snapshot directories for environment and requirements dumps."""
        snapshots_dir = root_dir / "snapshots"
        return {
            "snapshots": snapshots_dir,
            "environments": snapshots_dir / "environments",
            "requirements": snapshots_dir / "requirements",
        }

    def _dump_environment(self, root_dir: Path) -> None:
        ...
        try:
            ...
            snapshot_dirs = self._get_snapshot_dirs(root_dir)
            env_dir = snapshot_dirs["environments"]
            req_dir = snapshot_dirs["requirements"]

            save_path = self._get_versioned_path(env_dir / self.filename)
            req_path = self._get_versioned_path(req_dir / "requirements_frozen.txt")

            logger.info(f"📁 Environment snapshots directory: {env_dir}")
            logger.info(f"📁 Requirements snapshots directory: {req_dir}")

            # Create directories if needed
            if not env_dir.exists():
                logger.debug(f"Creating directory: {env_dir}")
                env_dir.mkdir(parents=True, exist_ok=True)
            if not req_dir.exists():
                logger.debug(f"Creating directory: {req_dir}")
