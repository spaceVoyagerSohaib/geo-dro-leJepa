            ...
            try:
                if (
                    ...
                    if key == "module" and "forward" in cfg[key]:
                        ...
                        # Ensure hparams are logged by default (include full resolved config)
                        if module_cfg.get("hparams") is None:
                            full_cfg = omegaconf.OmegaConf.to_container(cfg, resolve=True)
                            if isinstance(full_cfg.get("module"), dict):
                                full_cfg["module"].pop("hparams", None)
                            module_cfg["hparams"] = full_cfg
