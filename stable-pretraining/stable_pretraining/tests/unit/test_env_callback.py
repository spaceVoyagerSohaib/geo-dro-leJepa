...
def snapshot_env_path(base_dir: Path, filename: str = "environment.json") -> Path:
    """Return the expected environment snapshot path under base_dir."""
    base = Path(base_dir)
    return base / "snapshots" / "environments" / filename


def snapshot_req_path(
    base_dir: Path, filename: str = "requirements_frozen.txt"
) -> Path:
    """Return the expected requirements snapshot path under base_dir."""
    base = Path(base_dir)
    return base / "snapshots" / "requirements" / filename
...
class TestFileCreation:
    ...
    def test_files_created_sync(
        ...
        try:
            ...
            # Check files exist
            env_file = snapshot_env_path(trainer.default_root_dir)
            req_file = snapshot_req_path(trainer.default_root_dir)
            ...
            # Check files exist
            env_file = snapshot_env_path(trainer.default_root_dir, "environment_v1.json")
            req_file = snapshot_req_path(
                trainer.default_root_dir, "requirements_frozen_v1.txt"
            )
...
class TestStages:
    ...
    def test_only_runs_on_fit_stage(
        ...
        try:
            ...
            time.sleep(0.2)
            env_file = snapshot_env_path(temp_log_dir)
