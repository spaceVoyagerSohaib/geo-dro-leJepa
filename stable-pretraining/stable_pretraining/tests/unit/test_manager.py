...
class TestConfigureCheckpointing:
    ...
    def test_case_2b_resume_from_old_run_uses_user_callback_without_adding_new_one(
        self, manager_factory, tmp_path: Path
    ):
        """Resume path may differ from current save directory; keep user callback as source of truth."""
        resume_ckpt_path = tmp_path / "old_run" / "checkpoints" / "last.ckpt"
        resume_ckpt_path.parent.mkdir(parents=True)
        resume_ckpt_path.touch()

        current_run_ckpt_dir = tmp_path / "current_run" / "checkpoints"
        callbacks = [ModelCheckpoint(dirpath=str(current_run_ckpt_dir), save_last=True)]
        manager = manager_factory(
            callbacks=callbacks,
        ...
        initial_model_checkpoint_count = sum(
            isinstance(cb, ModelCheckpoint) for cb in manager._trainer.callbacks
        )

        manager._configure_checkpointing()

        final_model_checkpoint_count = sum(
            isinstance(cb, ModelCheckpoint) for cb in manager._trainer.callbacks
        )
        assert final_model_checkpoint_count == initial_model_checkpoint_count
...
@pytest.mark.unit
class TestManagerFitCall:
    """Tests how Manager forwards resume arguments into Trainer.fit."""

    def test_manager_forwards_resume_weights_only_to_fit(
        self, manager_factory, tmp_path: Path, monkeypatch
    ):
        ckpt_path = tmp_path / "checkpoints" / "last.ckpt"
        ...
        fit_mock = Mock()
        manager._trainer.fit = fit_mock
        manager.trainer = manager._trainer
        ...
        monkeypatch.setattr(manager, "_dump_wandb_data", lambda: None)

        manager()

        fit_mock.assert_called_once()
        _, kwargs = fit_mock.call_args
        assert kwargs["ckpt_path"] == str(ckpt_path.resolve())
