...
├── scripts/        # Manual/regression helpers
...
### Run only unit tests (used in CI)
```bash
python -m pytest -m unit
```

`python -m pytest` runs every discovered test under `stable_pretraining/tests`,
including integration tests, because `pytest.ini` sets `testpaths` to the full
test tree.
...
GeoDRO-LeJEPA coverage currently lives in the unit suite, especially
`unit/test_geodro_lejepa.py`, `unit/test_geodro_lejepa_eval.py`, and
`unit/test_run_manifest.py`.
